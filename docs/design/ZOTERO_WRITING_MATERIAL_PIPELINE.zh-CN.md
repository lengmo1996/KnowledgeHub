...
仍保留的边界：Docling `prov.charspan`不能外推到selection之外的全部论文、OCR和跨页自然段；非Docling/PyMuPDF fallback继续fail closed。`local reviewer only`已由独立`writing-material-rbac-v1`执行：正式CLI使用effective POSIX username+UID匹配0600 fingerprinted policy和固定角色权限，并继续叠加run目录private mode检查；这是单机POSIX信任域，不外推为跨主机IdP或MFA。到期处置采用plan→逐run cache purge→released-run alias-safe decommission→run/reference双quarantine→30天grace→inventory-verified purge；新cache原子绑定run scope，1281条legacy cache已保守迁移并完成scope fingerprint验证。Phase 14B2实现7个candidate/release目录和5个run独占physical collections的ownership检查、alias rollback/previous retirement、可恢复delete与本地隔离；Phase 14C实现协调intent/receipt、严格三步顺序、reference grace/purge和双quarantine coordinated purge。当前run未到期，真实索引、cache和目录均未处置；外部cron/systemd仅可在fingerprinted plan为ready时调用confirmation-gated命令，仓库不增加daemon或队列。生产rollback已于2026-07-19完成真实可逆演练：alias从quality-v2/1107切至旧v1/134并通过同一机制恢复，期间未创建snapshot、写collection或重建索引；该验证证明机制可用，但旧集合会恢复旧质量，仍须维护窗口和切换前后health query。
