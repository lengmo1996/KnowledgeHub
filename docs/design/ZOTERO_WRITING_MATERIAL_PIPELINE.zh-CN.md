...
仍保留的边界：Docling `prov.charspan`不能外推到selection之外的全部论文、OCR和跨页自然段；非Docling/PyMuPDF fallback继续fail closed；`local reviewer only`没有独立RBAC身份层。生产rollback已于2026-07-19完成真实可逆演练：alias从quality-v2/1107切至旧v1/134并通过同一机制恢复，期间未创建snapshot、写collection或重建索引；该验证证明机制可用，但旧集合会恢复旧质量，仍须维护窗口和切换前后health query。
