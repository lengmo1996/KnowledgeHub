...
- Qdrant 由可开机启用的 `knowledgehub-rag-core.service` 管理并等待健康；MCP unit
  `Requires=` 该 core unit。CPU-only Search API 随 core 自动启动；GPU embedding 和
  quality reranker 保持 `restart: "no"`，交互 workload 通过 static unit 按需领取
  显存，每日 RAG 则依据 `nvidia-smi` 临时选择单卡或双卡并在结束后释放。
