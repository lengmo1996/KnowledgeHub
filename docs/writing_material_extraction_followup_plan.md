...
## Phase 11：生产 rollback readiness（2026-07-19）

1. [x] 为`release rollback`增加显式`--dry-run`；与`--yes`组合拒绝，真实rollback仍必须单独确认。
2. [x] 只读核验promotion current state、live alias target、active/previous collection green/points/schema、release manifest fingerprint和snapshot availability。
3. [x] 生成fingerprinted `writing-material-rollback-readiness-v1`，明确`writes_performed=false`、`rollback_performed=false`和预期切换后状态。
4. [x] fixture覆盖ready、live alias drift blocked、snapshot查询、CLI safety flag及零snapshot/rollback调用。
5. [x] 真实演练返回ready，fingerprint `33f4505b05b97d75d113d6d6abf718009f4d9b79b68817c6effbfb215b7adc3f`；active 1107、previous 134，均green且schema一致，snapshot available。
6. [x] 演练后current仍为quality-v2、`rolled_back_at=null`，transaction仍为原promote/committed；未创建TaskStore、snapshot、collection或alias transaction。
7. [x] writing-material 165 passed、全仓549 passed、Ruff通过、mypy 129 source files通过。
...
## Phase 13：独立 POSIX 身份 RBAC（2026-07-19）

1. [x] 新增独立于pilot approval与run manifest的`writing-material-rbac-v1`策略；使用进程有效POSIX用户名+UID作为不可由CLI参数覆盖的认证主体。
2. [x] 定义`viewer`、`reviewer`、`operator`、`release_manager`、`retention_manager`、`administrator`固定角色及read/review/extract/index/release/retention-dispose/administer权限。
3. [x] 策略采用closed schema、canonical fingerprint、timezone issued_at；策略及父目录必须由当前有效UID持有、分别0600/0700，symlink、权限/owner漂移、未知角色、重复assignment、tamper均fail closed。
4. [x] 新增`writing-material access {bootstrap,status,check}`；bootstrap要求显式`--yes`、subject等于当前POSIX用户且拒绝覆盖既有策略。配置策略路径后，所有writing-material CLI按最小权限统一授权，缺失/非法策略直接拒绝。
5. [x] validation治理报告将RBAC授权与private filesystem permissions组合；未配置策略的历史fixture继续显示`identity_enforced=false`，不破坏旧运行资产。
6. [x] 真实策略已写入`/data/KnowledgeHub/writing-materials/access/rbac-policy.json`：目录0700、文件0600、owner `lengmo:lengmo`，roles=`administrator,reviewer`，fingerprint `0a3a9468b94a5548a36cda0bf9809cb47d298545ddbe1f679b091c5977d8fda9`。
7. [x] 当前30篇run真实只读validate为errors=[]、source verified、index eligible；治理报告`identity_enforced=true`，subject=`lengmo`、uid=1000、read permission granted。
8. [x] 正常授权、最小权限拒绝、未分配identity、tamper、closed-schema、权限漂移、symlink、bootstrap confirmation/overwrite、缺失policy fail-closed和CLI提前拒绝均有fixture；writing-material 174 passed、全仓558 passed、Ruff通过、mypy 130 source files通过、`git diff --check`通过。
...
## Phase 14C：协调式自动到期处置与双 quarantine purge（2026-07-20）

1. [x] 新增`WritingMaterialRetentionCoordinator`及fingerprinted coordinated plan/intent/receipt；`dispose`固定执行`cache scope purge → release retirement → run quarantine`，任一步失败不会越过后续安全前置条件。
2. [x] coordinator intent在首次写操作前绑定三个子计划及required状态；cache/release/run子receipt用于幂等恢复，支持cache已purge、release已retire、run已rename或coordinator receipt写入中断后的重复调用。
3. [x] 支持安全接管此前通过独立命令完成的run quarantine：只根据已验证子receipt补记coordinated intent/receipt，不重新运行已完成的alias/index/data mutation。
4. [x] 新增`writing-material-release-reference-purge-{plan,intent,receipt}-v1`；以retirement completed_at计算30天grace，purge前逐目录复验size/SHA-256 inventory，部分目录删除后中断可按intent恢复。
5. [x] coordinated purge同时等待run与release-reference两个grace；先清理reference quarantine，再清理run quarantine，子步骤已完成或中断时可安全重试。未发布run把reference purge标为`not_required`，不会永久阻塞。
6. [x] 已purged receipt不掩盖数据重新出现：run quarantine、reference quarantine或retired collection重新出现均fail closed；unscoped cache、新collection owner、alias drift和inventory drift继续阻断。
7. [x] CLI新增`plan-disposition`、`dispose --yes`、`plan-disposition-purge`、`purge-disposition --yes`，并保留`plan-reference-purge`/`purge-references --yes`作为单步运维入口；统一要求retention-dispose+release RBAC和derive/promotion/run三重锁。
...
Phase 14C结论：当前30篇pilot的自动到期处置从`PARTIAL`提升为`IMPLEMENTED + FIXTURE_VERIFIED + CURRENT_NOT_DUE`。系统不增加daemon、队列或外部依赖；外部cron/systemd只需周期调用只读`plan-disposition`，到期且ready后调用`dispose --yes`，宽限期后调用`purge-disposition --yes`。当前run未到期，未执行任何真实purge/decommission/quarantine。
